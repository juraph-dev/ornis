...
bool Ui::initialise(std::shared_ptr<Channel> interface_channel,
                    std::map<std::string, std::shared_ptr<StreamChannel>>& stream_map)
{
  ...
  interface_map_["nodes"] = std::unique_ptr<MonitorInterface>(new MonitorInterface("nodes", "[n]odes"));
  interface_map_["topics"] = std::unique_ptr<MonitorInterface>(new MonitorInterface("topics", "[t]opics"));
  interface_map_["services"] = std::unique_ptr<MonitorInterface>(new MonitorInterface("services", "[s]ervices"));
...
void Ui::renderMonitorInteractionResult(MonitorInterface* interface)
{
  ...
  ui_helpers::writeDetailedTreeToTitledPlane(
      *monitor_info_plane_, item, interface_channel_->request_response_trees_->second, options_.current_scheme_);
...
void Ui::renderMonitorSelection(MonitorInterface* interface)
{
  ...
  msg_tree::msg_contents request_contents = { .data_type_ = "", .entry_name_ = item, .entry_data_ = "" };

  // Create the msg trees to be used for storing the selection information
  currently_active_trees_ =
      std::make_shared<std::pair<msg_tree::MsgTree, msg_tree::MsgTree>>(request_contents, request_contents);
...
void Ui::refreshUi()
{
  ...
  while (screen_loop_)
  {
    ...
    // Handle re-draws in case of no input
    else
    {
      if (ui_displaying_ == UiDisplayingEnum::streamingTopic)
      {
        // While we're here, peform a check on our
        // stream plane, make sure it's centered
        // While we're here, we also want to center the stream plane
        const auto x = stream_map_->begin()->second->stream_plane_->get_x();
        const auto y = stream_map_->begin()->second->stream_plane_->get_y();
        const auto dim_x = stream_map_->begin()->second->stream_plane_->get_dim_x();
        const auto dim_y = stream_map_->begin()->second->stream_plane_->get_dim_y();
        const auto des_y = term_height_ / 2 - dim_y / 2;
        const auto des_x = term_width_ / 2 - dim_x / 2;
        if (std::abs(int(x - des_x)) >= 3 || std::abs(int(y - des_y)) >= 3)
        {
          stream_map_->begin()->second->stream_plane_->move(des_y, des_x);
        }
      }
    }
...
void Ui::handleInputOptions(const ncinput& input)
{
  if (input.id == 'q' || input.id == NCKEY_ESC ||
      (ui_helpers::mouseClick(input) && !checkEventOnPlane(input, options_.get_plane())))
  {
...
{
  if (input.id == 'q' || input.id == NCKEY_ESC ||
      (ui_helpers::mouseClick(input) && !checkEventOnPlane(input, interface_map_[selected_monitor_]->get_plane())))
  {
    transitionUiState(UiDisplayingEnum::monitors);
    return;
  }
...
void Ui::handleInputMonitorInteraction(const ncinput& input)
{
  ...
  if (!add_input)
  {
    auto editable_node = currently_active_trees_->first.getRoot()->getNthEditableNode(currently_editing_index_);
...
void Ui::handleInputMonitorSelection(const ncinput& input)
{
  if (input.id == NCKEY_ESC || input.id == 'q' ||
      (ui_helpers::mouseClick(input) && !checkEventOnPlane(input, monitor_info_plane_.get())))
  {
    transitionUiState(UiDisplayingEnum::selectedMonitor);
    return;
  }
...
void Ui::handleInputMonitors(const ncinput& input)
{
  ...
  if (input.id == 't' ||
      (ui_helpers::mouseClick(input) && checkEventOnPlane(input, interface_map_["topics"]->get_plane())))
  {
    selected_monitor_ = "topics";
    transitionUiState(UiDisplayingEnum::selectedMonitor);
  }
  else if (input.id == 'n' ||
           (ui_helpers::mouseClick(input) && checkEventOnPlane(input, interface_map_["nodes"]->get_plane())))
  {
    selected_monitor_ = "nodes";
    transitionUiState(UiDisplayingEnum::selectedMonitor);
  }
  else if (input.id == 's' ||
           (ui_helpers::mouseClick(input) && checkEventOnPlane(input, interface_map_["services"]->get_plane())))
  {
    selected_monitor_ = "services";
    transitionUiState(UiDisplayingEnum::selectedMonitor);
  }
  else if (input.id == 'o' ||
           (ui_helpers::mouseClick(input) && checkEventOnPlane(input, options_.minimised_plane_.get())))
  {
...
void Ui::renderHomeLayout()
{
  ...
  switch (layout)
  {
    case UiLayoutEnum::Horizontal: {
      ...
      // Place Node monitor between topics and services. It may be off-center, but placing at mid-point can cause
      // overlap
  ...
  plane_loc_vector.push_back(
      std::tuple<const ncpp::Plane*, int, int>(interface_map_.at("topics")->get_plane(), topic_x, topic_y));

  plane_loc_vector.push_back(
      std::tuple<const ncpp::Plane*, int, int>(interface_map_.at("nodes")->get_plane(), node_x, node_y));

  plane_loc_vector.push_back(
      std::tuple<const ncpp::Plane*, int, int>(interface_map_.at("services")->get_plane(), service_x, service_y));
...
void Ui::renderSelectedMonitor()
{
  ...
  std::sort(interface_order.begin(), interface_order.end(), [](const MonitorInterface* a, const MonitorInterface* b) {
  ...
  planes_locations.push_back(std::tuple<const ncpp::Plane*, int, int>(right_hand_plane, term_width_, 1));
  // Place selected plane in center of screen
  planes_locations.push_back(
      std::tuple<const ncpp::Plane*, int, int>(selected_plane, term_width_ / 2 - selected_plane->get_dim_x() / 2,
...
void Ui::openStream(const std::string& topic_name, const std::string& entry_path,
...
void Ui::transitionUiState(const UiDisplayingEnum& desired_state)
{
  ...
  switch (desired_state)
  {
    ...
    case UiDisplayingEnum::selectedMonitor: {
      ...
      else if (ui_displaying_ == UiDisplayingEnum::monitorInteraction ||
               ui_displaying_ == UiDisplayingEnum::monitorInteractionResult ||
               ui_displaying_ == UiDisplayingEnum::monitorSelection)
      {
        monitor_info_plane_->erase();
    ...
    case UiDisplayingEnum::monitorEntry: {
      ...
      {
        ui_helpers::drawHelperBar(notcurses_stdplane_.get(), userHelpStrings_.interactable_entry_prompt);
      }
      ...
      {
        ui_helpers::drawHelperBar(notcurses_stdplane_.get(), userHelpStrings_.standard_entry_prompt);
      }
    ...
    case UiDisplayingEnum::monitorSelection: {
      if (ui_displaying_ == UiDisplayingEnum::streamingTopic)
      {
        const std::string selected_entry = interface_map_[selected_monitor_]->selector_->get_selected();
    ...
    case UiDisplayingEnum::streamingTopic: {
      ...
      const auto selected_path = currently_active_trees_->first.getRoot()->getPathNthNode(currently_editing_index_);
    ...
    case UiDisplayingEnum::optionsMenu: {
      ui_helpers::drawHelperBar(notcurses_stdplane_.get(), userHelpStrings_.options_prompt);
      renderOptions();
...
void Ui::movePlanesAnimated(const std::vector<std::tuple<const ncpp::Plane*, const int, const int>>& planes_locations)
{
  // Construct the vector transition locations
  std::vector<std::pair<const ncpp::Plane*, std::vector<std::pair<const int, const int>>>> plane_trajectories;
  ...
  for (const auto& planes_to_move : planes_locations)
  {
    ...
    const auto plane_pair =
        std::pair<const ncpp::Plane*, std::vector<std::pair<const int, const int>>>(plane, plane_trajectory);
...
{
  if ((ui_helpers::mouseClick(input) && checkEventOnPlane(input, interface->get_plane())) ||
      ui_helpers::selectInput(input))
  {
...
{
  return (input.x > plane->get_x() && input.x < (int)plane->get_dim_x() + plane->get_x() && input.y > plane->get_y() &&
...
void Ui::renderOptions()
{
  ...
  for (const auto& interface : interface_map_)
  {
    ...
    planes_locations.push_back(std::tuple<const ncpp::Plane*, int, int>(interface.second->get_plane(), x, y));
  }
  ...
  for (const auto& interface : interface_map_)
  {
    ...
    interface.second->get_plane()->move(y >= (int)term_height_ / 2 ? -40 : term_height_ + 40,
                                        x == 1 ? -40 : term_width_ + 40);
  }
}
