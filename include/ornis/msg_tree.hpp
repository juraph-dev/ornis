...
public:
  ...
  MsgTreeNode(const msg_contents& new_msg, const MsgTreeNode* parent) : msg_contents_(new_msg), parent_(parent)
  {
  }
  ...
  void setValue(const std::string& entry_data)
  {
    this->msg_contents_.entry_data_ = entry_data;
  }

  void setEditingStatus(const bool& edit_status)
  {
    being_edited_ = edit_status;
  }

  bool getEditingStatus() const
  {
    return being_edited_;
  }

  msg_contents& getValue()
  {
    return this->msg_contents_;
  }

  const msg_contents& getValue() const
  {
    return this->msg_contents_;
  }

  std::vector<MsgTreeNode>& getChildren()
  {
    return this->children_;
  }

  const std::vector<MsgTreeNode>& getChildren() const
  {
    return this->children_;
  }
  ...
  const MsgTreeNode& getChild(size_t index) const
  {
    return this->children_[index];
  }

  MsgTreeNode& getChild(size_t index)
  {
    return this->children_[index];
  }

  const MsgTreeNode* getParent()
  {
    return parent_;
  }
  ...
  void writeNodeToMessage(uint8_t* message_data, const rosidl_typesupport_introspection_cpp::MessageMembers* members)
  {
...
public:
  ...
  const MsgTreeNode* getRoot() const
  {
    return base_.get();
  }
  MsgTreeNode* getRoot()
  ...
  const MsgTreeNode* getFirstEditableNode() const
  {
    return base_->getNthEditableNode(1);
  }

  void recursivelyCreateTree(MsgTreeNode* target_node, const rosidl_message_type_support_t* type_data)
  {
    ...
    for (size_t i = 0; i < members->member_count_; i++)
    {
      ...
      const msg_contents msg_data = { .data_type_ = new_node_type, .entry_name_ = member.name_, .entry_data_ = "" };

      MsgTreeNode* new_node = target_node->addChild(msg_data);

      if (member.is_array_)
      {
        const msg_contents msg_array_data = { .data_type_ = "Array", .entry_name_ = "[]", .entry_data_ = "" };
    ...
    for (size_t i = 0; i < members->member_count_; i++)
    {
      ...
      if (member.is_array_)
      {
        ...
        for (uint32_t i = 0; i < array_size; ++i)
        {
          ...
          const uint8_t* raw_array_element_data =
              static_cast<uint8_t*>(member.get_function(message_data + member.offset_, i));
          introspection::messageDataToString(member, raw_array_element_data, array_element_data);
      ...
      else
      {
        // Node has no chldren, probably editable
        editable_node_count_++;
        introspection::messageDataToString(member, &message_data[member.offset_], new_node->getValue().entry_data_);
      }
      node_count_++;
    }
  }

  void writeTreeToMessage(uint8_t* message_data,
