...
void ServiceMonitor::getEntryInfo(const std::string& entry_name, const std::string& entry_details,
                                  std::map<std::string, std::vector<std::string>>& entry_info)
{
  (void)entry_name;

  msg_tree::msg_contents req_contents = { .data_type_ = "", .entry_name_ = "Request", .entry_data_ = "" };

  msg_tree::msg_contents res_contents = { .data_type_ = "", .entry_name_ = "Response", .entry_data_ = "" };
...
void ServiceMonitor::getInteractionTrees(const std::string service_type, msg_tree::MsgTree& request,
                                         msg_tree::MsgTree& response)
{
  ...
  service_info_.type_support =
      introspection::getServiceTypeSupport(service_type, rosidl_typesupport_introspection_cpp::typesupport_identifier);

  const auto* service_members = static_cast<const ServiceMembers*>(service_info_.type_support->data);
  ...
  service_info_.request_type_support =
      introspection::getMessageTypeSupport(request_char, rosidl_typesupport_introspection_cpp::typesupport_identifier);

  service_info_.response_type_support =
      introspection::getMessageTypeSupport(response_char, rosidl_typesupport_introspection_cpp::typesupport_identifier);
...
void ServiceMonitor::interact(const std::string& entry_name, const std::string& entry_details,
                              const msg_tree::MsgTree& request, msg_tree::MsgTree& response)
{
  ...
  auto* request_members = static_cast<const rosidl_typesupport_introspection_cpp::MessageMembers*>(
      service_info_.request_type_support->data);

  // Allocate space to store the message
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  uint8_t* request_data = static_cast<uint8_t*>(allocator.allocate(request_members->size_of_, allocator.state));
  ...
  auto ret = rcl_client_init(&client, ros_interface_node_.get(), type_support, service_name, &client_ops);
  ...
  auto* response_members = static_cast<const rosidl_typesupport_introspection_cpp::MessageMembers*>(
      service_info_.response_type_support->data);

  uint8_t* response_data = static_cast<uint8_t*>(allocator.allocate(response_members->size_of_, allocator.state));

  rmw_service_info_t req_header;
  rcl_wait_set_t wait_set = rcl_get_zero_initialized_wait_set();
  ret = rcl_wait_set_init(&wait_set, 0, 0, 0, 1, 0, 0, ros_interface_node_->context, rcl_get_default_allocator());
  ...
  msg_tree::msg_contents response_contents = { .data_type_ = "", .entry_name_ = "response", .entry_data_ = "" };
...
void ServiceMonitor::updateValue()
{
  ...
  int ret = rcl_get_service_names_and_types(ros_interface_node_.get(), &allocator, &service_names_and_types);
  ...
  for (size_t i = 0; i < service_names_and_types.names.size; ++i)
  {
    ...
    for (size_t j = 0; j < service_names_and_types.types[i].size; ++j)
    {
      service_info.push_back(
          std::pair<std::string, std::string>{ service_name, service_names_and_types.types[i].data[j] });
    }
  }
