...
ObjectController::~ObjectController() {
  ...
  if (ret != RCL_RET_OK) {
    std::cerr << "Error occurred deleting object monitor rcl_node! Error: "
              << ret << " \n";
  }
}
...
void ObjectController::checkUiRequests() {
  ...
  if (interface_channel_->request_pending_.load()) {
    ...
    case (Channel::RequestEnum::monitorEntryInformation): {
      ...
      monitor_map_[interface_channel_->request_details_["monitor_name"]]
          ->getEntryInfo(interface_channel_->request_details_["monitor_entry"],
                         entry_details, interface_channel_->response_map_);
    ...
    case (Channel::RequestEnum::topicStreamer): {
      ...
      // (Both to and from)
      stream_interface_map_[topic_name] =
    ...
    case (Channel::RequestEnum::closeStream): {
      ...
      const auto &stream_name =
          interface_channel_->request_details_["stream_name"];
...
int ObjectController::spin() {
  ...
  ros_interface_node_ =
      std::make_shared<rcl_node_t>(rcl_get_zero_initialized_node());
  ret = rcl_node_init(ros_interface_node_.get(), "ornis", "", &context_,
                      &node_options);
  ...
  const auto ts = introspection::getMessageTypeSupport(
      "std_msgs/msg/String",
