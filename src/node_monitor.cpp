  ...
  std::map<std::string, std::vector<std::string>> & entry_info)
{
  ...
  // Publishers
  auto ret = rcl_get_publisher_names_and_types_by_node(
    ros_interface_node_.get(), &allocator, false, entry_name.c_str(), blank_namespace,
    &published_topics);
  if (ret == 0) {
  ...
  ret = rcl_get_service_names_and_types_by_node(
    ros_interface_node_.get(), &allocator, entry_name.c_str(), blank_namespace, &service_servers);
  ...
  ret = rcl_get_client_names_and_types_by_node(
    ros_interface_node_.get(), &allocator, entry_name.c_str(), blank_namespace, &service_clients);
  ...
  ret = rcl_action_get_client_names_and_types_by_node(
    ros_interface_node_.get(), &allocator, entry_name.c_str(), blank_namespace, &action_clients);
  ...
  ret = rcl_action_get_server_names_and_types_by_node(
    ros_interface_node_.get(), &allocator, entry_name.c_str(), blank_namespace, &action_servers);
...
void NodeMonitor::namesAndTypesToMap(
