...
void TopicMonitor::getEntryInfo(const std::string& entry_name, const std::string& entry_details,
                                std::map<std::string, std::vector<std::string>>& entry_info)
{
  (void)entry_details;
  rcl_allocator_t allocator = rcl_get_default_allocator();
  rcl_topic_endpoint_info_array_t topic_publishers = rcl_get_zero_initialized_topic_endpoint_info_array();
  rcl_topic_endpoint_info_array_t topic_subscribers = rcl_get_zero_initialized_topic_endpoint_info_array();

  int ret = rcl_get_publishers_info_by_topic(ros_interface_node_.get(), &allocator, entry_name.c_str(), false,
                                             &topic_publishers);
  ...
  ret = rcl_get_subscriptions_info_by_topic(ros_interface_node_.get(), &allocator, entry_name.c_str(), false,
                                            &topic_subscribers);
...
void TopicMonitor::getInteractionTree(const std::string message_type, msg_tree::MsgTree& message_tree)
{
  const rosidl_message_type_support_t* message_type_support =
      introspection::getMessageTypeSupport(message_type, rosidl_typesupport_introspection_cpp::typesupport_identifier);
...
void TopicMonitor::updateValue()
{
  ...
  int ret = rcl_get_topic_names_and_types(ros_interface_node_.get(), &allocator, false, &topic_names_and_types);
  ...
  for (size_t i = 0; i < topic_names_and_types.names.size; ++i)
  {
    ...
    {
      topic_info.push_back(std::pair<std::string, std::string>{ service_name, topic_names_and_types.types[i].data[j] });
    }
  }
  ret = rcl_names_and_types_fini(&topic_names_and_types);
  if (ret != RCL_RET_OK)
  {
    std::cerr << "Topic monitor failed to finish topic names and types!\n";
  }
